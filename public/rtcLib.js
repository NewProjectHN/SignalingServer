...
let user = null;
let callback = null;
function createSocketRTC(config,user,callback){
  ...
  socket.on('connect', function(data) {
    console.log('connect');
    let {getFriendOnlineCB} = callback;
    socket.emit('join', user,getFriendOnlineCB);
  });
  ...
  // socket.on("end-call-from", function(user) {
  ...
  socket.on("send-msg", function(data) {
    $scope.addNewMessage(false,data.userId,data.msg);
    // $scope.callFrom = frienduserId;
    $scope.$apply();
  });
...
function createPeerConnection(user, isOffer) {
  ...
  retVal.onaddstream = function (event) {
    ...
    let {startCallSuccessCB} = parent.callback;
...
function makeCall(user,localStream){
...
function changeLocalStream(localStream){
  this.localStream = localStream;
}
